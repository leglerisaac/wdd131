...
function searchObjectList(list, search) {
  function searchCallback(object) {
    return (
      ...
      object.tags.find((tag) => tag.toLowerCase().includes(search.toLowerCase()))
    );
  }
  const filteredList = list.filter(searchCallback);
  const sortedList = filteredList.sort((a, b) => a.distance > b.distance);
  return sortedList;
}

console.log(searchObjectList(hikes, "al"));
console.log(searchObjectList(hikes, "Menan"));
console.log(searchObjectList(hikes, "steep"));
